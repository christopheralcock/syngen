...
class SynGen < Sinatra::Base
  ...
  get '/' do
    ...
    @plot = [lifeTurnedUpsideDown, emotionalJourney, aMeetsB, biopic, documentary].sample

    @funnyBits = ["3 stars, some funny bits", "", "", "", "", "", "", "", "", ""].sample

    sites = ["Picturehouse venues", "car parks", "school halls", "IMAX theatres"]

    @presentation = "Presented in #{rand(8...180)}mm at selected #{sites.sample}"
